Aet Core version *0.15.1* is now available from:

  <https://download.aet.org/aet-0.15.1/>
...
A number of changes to the way Aet Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 maet per kB (0.00001 AET). This is to prepare for dropping the minimum transaction fee to 0.1 maet per kB (0.0001 AET) in the next release.
