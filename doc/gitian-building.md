...
*Setup instructions for a Gitian build of Aet Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aet
...
- [Building Aet Core](#building-aet-core)
...
Clone the git repositories for aet and Gitian.
...
in the aet repository under 'Fetch and create inputs' to install sources which require
...
To build Aet Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the aet repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aet/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/aet-project/aet
...
For example:
```bash
URL=https://github.com/thrasher-/aet.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aet=${COMMIT} --url aet=${URL} ../aet/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aet=${COMMIT} --url aet=${URL} ../aet/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aet=${COMMIT} --url aet=${URL} ../aet/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the aet git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../aet/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/aet
SIGPATH=/some/root/path/aet-detached-sigs

./bin/gbuild --url aet=${BTCPATH},signature=${SIGPATH} ../aet/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aet-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aet-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/aet-osx-build.assert
```
...
[aet-project/gitian.sigs.ltc](https://github.com/aet-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
