Aet Core version 0.13.2 is now available from:

  <https://download.aet.org/aet-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/aet-project/aet/issues>
...
updates will be released anymore. Without security updates, using a aet
...
ECDSA signatures inside Aet transactions now use validation using
...
Previous versions of Aet Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Aet Core was to raise the default minimum
relay fee.

Aet Core 0.13.2 will have a strict maximum size on the mempool. The
...
Aet Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running aetd without having to do any manual configuration.
...
Aet Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Aet Core will relay transactions with
...
entirely in Aet Core 0.13, and it is at this time undecided whether the
...
Aet Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Aet Core automatically creates a hidden service to listen on, without
manual configuration. Aet Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Aet Core is listening, and
...
Aetd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Aet
Core to use floating fees. By default, Aet Core will use floating
fees.
...
(default: `0.0002` AET/kB).

At all times, Aet Core will cap fees at `-maxtxfee=<x>` (default:
0.10) AET.
Furthermore, Aet Core will never create transactions paying less than
...
`aet.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /aetrpc>
        ...
        ProxyPassReverse http://127.0.0.1:9933/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass aetd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< aetrpc:<password>
    </Location>
...
Restarting aetd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in aet.conf
...
For example:
    ...
    $ src/aet-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Aet Core code base has
...
- `aet-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `aet-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Aet scripting system that in combination with
...
Aet transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Aet Core.
...
The two phases are distinct in the Aet-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Aet wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Aet block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of aets they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of aets being spent is exactly
...
- **Increased security for multisig:** Aet addresses (both P2PKH addresses
  ...
  believed to be provided by Aet's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Aet's current ledger (called the UTXO set),
  ...
  note that Aet Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Aet users to individually opt-in to almost any change in the Aet
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Aet Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Aet
client to Aet Core 0.13.2 to signal for segwit support. When a
...
Since Aet Core 0.10.0, nodes have defaulted to only relaying and
...
- `aet-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `aet-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
